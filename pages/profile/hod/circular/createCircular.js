...
const today = new Date();
    function toggleDisplay(item){
        if(circularDisplay.includes(item)){
            const arr = []
            circularDisplay.map((cir)=>{
                if(cir != item){
                    arr.push(cir)
                }
            })
            setCircularDisplay(arr)
        }
        else{
            const arr = []
            arr.push(item)
        ...
        console.log("cir",circularDisplay)
    }
  return (
    <div className='min-h-screen bg-white flex  '> 
    <div className='w-[30%]  bg-gray-800 h-screen flex-none fixed top-0 left-0 overflow-hidden pr-4'><div className="  sticky bg-white overflow-hidden">
      <div className=" bg-gray-800 text-white   ">
        <div className="p-[4%] text-white ">
          <p className="text-white text-center font-bold text-[25px] tracking-wide">
            Enter Details
          </p>
          <form action="../../../api/circular" method='POST' onSubmit={()=>{
            console.log("inside submit")
          }}>
            {console.log("user",user)}
            <input type="text"  className='hidden' name='collegeCode' value={user.college.paraphrase}  />
            <input type="text"  className='hidden' name='department' value={user.department}  />
            <input type="text"  className='hidden' name='postedBy' value={user._id}  />
          <div className="py-3 px-7">
            ...
            <div className="py-2 ">
              ...
              <textarea
                ...
                placeholder="Enter body of the circular"
                // maxLength={350}
                value={circularBody}
                name='circularBody'
            ...
            <div className="py-2">
              ...
              <div>
                <input
                  ...
                  name="pos"
                  value="Director"
                  onChange={(e)=>{toggleDisplay(e.target.value)}}
                />
    ...
    {/* right  */}
    <div className='w-[80%] flex-auto ml-[550px] scale-[85%]'><div className=" bg-slate-50 w-[210mm] flex-col my-8 h-[270mm] overflow-auto drop-shadow-2xl flex  min-w-[210mm]">
        {/* header */}
